...
// Don't copy directories that are Raspberry-Pi serial number
var filter = function(stat, filepath, filename) {
    var reg = new RegExp(/[1-9a-f]{8}/);
    if (stat === 'directory' && reg.test(filename)) {
        return false;
    }
    return true;
}
...
Server.prototype.requestListener = function(req, res) {
    ...
    if (serial_no === filename) {
        ...
        if (!fs.existsSync(serial_dir)) {
            ...
            copydir.sync(this.root, serial_dir, filter, function(err) {
                if (err) {
                    console.log(err);
                }
            });
